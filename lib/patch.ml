...
let rec rewrite_name_expr source replacement bound = function
  ...
  | EBroadcast (typ, payload) ->
      EBroadcast (typ, rewrite_name_expr source replacement bound payload)
...
let rec replace_first_expr target replacement expr =
  if expr = target then (replacement, true)
    ...
    | EBroadcast (typ, payload) ->
        let payload, changed = replace_first_expr target replacement payload in
        (EBroadcast (typ, payload), changed)
    | EBind (process, name, typ, body) ->
...
let patch_audits_dir = Patch_audit.patch_audits_dir

let patch_latest_path = Patch_audit.patch_latest_path

let patch_audit_path = Patch_audit.patch_audit_path
...
let audit_of_patch_audit (audit : Patch_audit.audit) =
  {
...
let verify_audit ?(ref = "latest") store_root =
  try Patch_audit.verify_audit ~ref store_root |> audit_of_patch_audit with
  | Patch_audit.Error msg -> fail msg

let verify_latest_matches_store store_root =
  try Patch_audit.verify_latest_matches_store store_root |> audit_of_patch_audit with
  | Patch_audit.Error msg -> fail msg
