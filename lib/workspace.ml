...
let rec type_refs = function
  ...
  | TList t | TView t | TAttr t | TProcess t | TSecretRef (_, t) -> type_refs t
  | TVar _ -> []
...
let rec expr_type_refs = function
  ...
  | ERecordUpdate (record, updates) ->
      expr_type_refs record @ List.concat_map (fun (_, e) -> expr_type_refs e) updates
...
let package_alias manifest target =
...
let package_import_manifest manifest (name, path) =
  ...
  | None -> ()
  | Some (alias_name, _) -> (
      match package_alias_name_version alias_name with
      | None -> fail ("package alias must be package@semver: " ^ alias_name)
      | Some (expected_name, expected_version) ->
          if not (String.equal expected_name imported.name) then
            fail
              ("package alias name mismatch: expected " ^ expected_name ^ ", got "
             ^ imported.name)
