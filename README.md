...
- Files can use a small Elm-like surface syntax for top-level `name : Type` / `name = expr` / `name arg = expr` definitions, typed local `let` bindings, `type alias`, indented union types, record type/value literals, list literals when an expected `List A` is available, field access like `model.count`, lambdas written `\x y -> expr`, whitespace application, Nat addition with `a + b`, signature-free Nat additions such as `add a b = a + b`, `if ... then ... else ...`, `let ... in`, `case ... of`, and pipelines such as `value |> f |> g`. This surface is converted to the existing canonical S-expression AST before checking, so equivalent S-expression and Elm-like files hash the same.
- Cursor/VS Code syntax highlighting and Ctrl+Click go-to-definition for `.protoss` files is available in `editors/cursor/protoss-syntax`; copy it to `~/.cursor/extensions/protoss.protoss-syntax-0.2.1`, and to `~/.cursor-server/extensions/protoss.protoss-syntax-0.2.1` when using Cursor Remote/WSL, then restart Cursor.
...
- The self-hosted frontend is reachable from the CLI with `protoss self parse|resolve|deps|capabilities|static <file>` (add `--json` to `static`) and `protoss self fmt [--check] <file>`. Each command splices the target source into a driver definition, checks the combined program with the trusted OCaml kernel, and evaluates the matching `stdlib/prelude.protoss` function (`Protoss.selfParseJson`, `Protoss.selfResolveJson`, `Protoss.selfDepsJson`, `Protoss.selfCapabilitiesJson`, `Protoss.selfStaticJson`/`Protoss.selfStaticText`, `Protoss.formatText`) through the normal evaluator — so the report is produced by Protoss code, rendered with `Json.render`, while canonical DefIds still come from the kernel. `self static --json` includes the kernel-computed `frontendDefId` of the frontend implementation. The formatter is deterministic and idempotent (`fmt(fmt(x)) = fmt(x)`) and preserves DefIds after canonicalization. `PROTOSS_STDLIB` overrides the prelude path. See `docs/self-hosting.md` for the trusted-kernel boundary, and `conformance/self_host/` for golden parse/resolve/deps/capabilities/static/format/diagnostic outputs.
