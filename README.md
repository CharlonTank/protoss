...
- `defrec` supports structural Nat/List/Variant recursion and desugars to `foldNat`, `foldList`, or `foldVariant`; `defrecpoly` does the same for polymorphic structural definitions. Malformed, self-recursive, or non-structural `recur` definitions are rejected.
