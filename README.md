...
- `defrec` supports structural Nat/List/Variant recursion and desugars to `foldNat`, `foldList`, or `foldVariant`; `defrecpoly` does the same for polymorphic structural definitions. Variant recursion can recurse over direct recursive payload fields, nested record payload fields, and recursive fields of record items taken from direct payload lists. Malformed, self-recursive, or non-structural `recur` definitions are rejected.
