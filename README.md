...
- The shipped prelude includes polymorphic `List.map`, `List.length`, `List.append`, `List.filter`, `Maybe.map`, `Maybe.withDefault`, `Maybe.isSome`, `Maybe.isNone`, `Maybe.andThen`, `Result.map`, `Result.withDefault`, `Result.mapError`, `Result.andThen`, `Pair.swap`, `Assoc.empty`, `Assoc.insert`, `Assoc.get`, `Assoc.contains`, `Assoc.keys`, `Assoc.values`, `Set.empty`, `Set.contains`, `Set.insert`, and `Set.union`, plus monomorphic Nat/Bool/String helpers.
