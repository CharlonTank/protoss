...
- `.pth` harness files use `harness name = example def` for executable examples, `harness name = unit def == expected` for normalization assertions, `property prop [with generator]` for generated property checks, plus `generator`, `benchmark`, `invariant`, `migration`, `scenario`, `security`, `diagnostic`, and `ai-eval` entries. Harness refs are `HarnessId = H(canonicalBytes(harness))` and project builds store a canonical `protoss-harness-graph-v1` in `.protoss/store/harness.graph.json`, linked from `UniverseRoot`. `protoss harness run <project-or-store> <harness.pth>` runs a harness against a project build or a store `program.graph.json` and prints a JSON pass/fail report with per-harness IDs.
...
- `defrec` supports structural Nat/List/Variant recursion and desugars to `foldNat`, `foldList`, or `foldVariant`; `defrecpoly` does the same for polymorphic structural definitions. Variant recursion can recurse over direct recursive payload fields, nested record payload fields, and recursive fields of record items taken from direct payload lists. The termination explanation reports static type-size metadata (`staticTypeNodes`, `staticArrowArity`, and sized Nat/List/Variant/record arguments) for the checked definition. Malformed, self-recursive, or non-structural `recur` definitions are rejected.
- Productive corecursion is explicit and finite-observable: `(Stream A)` values are built with `(coiter State A seed step)`, where `step : State -> { head : A, state : State }`, and observed with `streamHead`, `streamTail`, or finite `streamTake`. Explicit `(Automaton State Output)` values use `(automaton State Output initial transition)`, where `transition : State -> { output : Output, state : State }`, and `automatonRun` returns a finite output prefix. The kernel rejects missing next-state fields by ordinary typechecking.
- Web apps are checked by convention with either the Process architecture (`init : Process Model`, `update : Msg -> Model -> Process Model`, `view : Model -> View Msg`) or the command architecture (`init : (Tuple Model (Cmd caps Msg))`, `update : Msg -> Model -> (Tuple Model (Cmd caps Msg))`, `view : Model -> View Msg`). The current command runtime treats `unit` as `Cmd.none` and ignores the command slot while preserving the typed `Cmd (capabilities ...) Msg` contract.
- `examples/web/site_vitrine` is a complete Protoss/H web app fixture using human syntax for model, messages, update, view composition, lists, records, variants, and app-checkable web conventions.
- Web builds write `protoss-compiled-artifact.txt` and embed `compiledArtifact` in `protoss-app.json`; the artifact ref is `derive(UniverseRoot, Target, OptimizationPolicy)` using target `web` and policy `web-default-v1`. `project build --target bytecode|wasm|llvm|javascript|sql-dataflow|gpu-kernel` writes deterministic content-addressed backend manifests under `.protoss/store/compiled/` for the Protoss VM bytecode, WebAssembly, LLVM/native, standalone JavaScript, SQL/dataflow, and GPU-kernel targets. The `bytecode` target additionally emits the real encoded Protoss VM module as a deterministic `.ptvm` file (equal to `Bytecode.encode_module` of the compiled program, decode-round-trip stable and at VM/interpreter parity); `protoss bytecode exec <file.ptvm> --entry <name>` then runs a built module directly with no source, at interpreter parity on the pure fragment. The other backend targets remain manifest-only stubs until their codegen lands.
...
- `protoss fmt [--check] <file>` prints the trusted S-expression AST projection for any parseable human source. Formatting is idempotent over the supported declaration grammar and preserves parseability.
- `protoss fmt --human [--check] <file>` renders the Elm-like Protoss/H projection of the parsed program (`Surface_syntax.render_program`): sugar — `if`/`case ... of`/`let ... in` layout, records, record updates, lists, operators, lambdas, field access, `Process { Cap } A` signatures — wherever re-parsing provably reconstructs the same canonical term, and plain parenthesized juxtaposition for the remaining special forms. The command carries a hard view/canon guard: it refuses to emit text whose re-parse does not hash identically to the source (for files that check in isolation), and rendering is idempotent. Programs with no Protoss/H projection are rejected with an explicit error instead of emitting text that would parse differently: `defpoly`/`defrecpoly` definitions, `defcap` declarations whose type does not carry the matching explicit `Process { ... }` scope, the single-atom `(Clock.read)` request, a `case`/`let` in inline (non-tail) position, function types in expression position, and lowercase dotted names that would re-parse as field access.
...
- `bench build <project>` writes a `protoss-benchmark-v1` report under `.protoss/store/benchmarks/<benchmark-ref>.benchmark` and prints the content-addressed `benchmark-ref`.
- Patch diagnostics include patch file paths, JSON syntax `line:column` locations, the failing operation number, operation kind, definition name, field context, and embedded `expr.source` line/column when kernel type errors can be mapped back to that source. The structural patch ADT accepts `AddDef`, `ReplaceDef`, `DeleteDef`, `RenameDef`, `AddField`, `RemoveField`, `Inline`, `Extract`, `AddHarness`, `AddCapability`, and `MigrateType`; every variant still goes through whole-program validation and audit recording. `patch from-text-diff <store> <diff.patch>` converts a single unambiguous human text diff into structural `AddDef`, `ReplaceDef`, or `DeleteDef` JSON and rejects multi-definition or rename-shaped textual edits with an intent error. `patch review` renders a human-readable operation summary before validation or apply. Successful `patch apply` writes a deterministic content-addressed audit file under `store/patches/<patch-ref>.patch`, links it to the previous audit with `previous-ref`, records `previous-root`/`root-ref`, writes content-addressed root-state and patch-provenance records under `store/provenance/`, records the same patch provenance as a `patch-provenance` event in `store/provenance/world-ledger`, and updates `store/patches/latest`; `patch audit` verifies and prints that chain, and the default `latest` audit must match the current store program hash and latest root state. Project `audit` also verifies the latest patch audit when present. Rejected patches do not write audit artifacts.
- The native agent protocol is `AI -> PatchCandidate -> Validator -> Harness -> Commit`. `protoss agent protocol` prints the versioned contract, `protoss agent guard-write <path>` rejects direct writes to canonical/store internals, `protoss agent factor-identical <project-or-store> [--out <patch.json>]` proposes a structural `DeleteDef` patch for duplicate DefIds whose source-level dependents do not need rewriting, `protoss agent synthesize-tests <project-or-store>` emits deterministic harness/test suggestions from checked definition types, `protoss agent generate-migration <old-project-or-store> <new-project-or-store>` emits a `migrate_v1_v2` patch candidate for detected web Model shape changes, including nested record migrations that copy unchanged nested fields and default newly introduced nested fields, `protoss agent compare-candidates <project-or-store> <left.patch.json> <right.patch.json>` checks two patch candidates without mutating the store and reports validity/result diagnostics, and `protoss agent commit <store> <patch.json> --harness <harness.pth>` is the agent mutation path through patch validation, attached harness pass/fail validation, and content-addressed audit. Agent commits reject missing or failing harnesses before mutating the store.
...
dune exec protoss -- harness run examples/harness_project examples/harness_project/harness/smoke.pth
...
dune exec protoss -- patch from-text-diff before.store human.diff > patch.json
...
dune exec protoss -- ledger compare-branches <ledger-root> harness/smoke.pth feature control
...
dune exec protoss -- agent synthesize-tests examples/web/todo_app/.protoss/store
dune exec protoss -- agent generate-migration /tmp/old-store /tmp/new-store
dune exec protoss -- agent compare-candidates examples/web/todo_app/.protoss/store left.patch.json right.patch.json
dune exec protoss -- agent commit examples/harness_project/.protoss/store examples/harness_project/patches/add_three.json --harness examples/harness_project/harness/smoke.pth
