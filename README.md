...
- Files can use a small Elm-like surface syntax for top-level `name : Type` / `name = expr` definitions, `type alias`, indented union types, record type/value literals, lambdas written `\x -> expr`, whitespace application, and pipelines such as `value |> f |> g`. This surface is converted to the existing canonical S-expression AST before checking, so equivalent S-expression and Elm-like files hash the same.
