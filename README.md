...
- `project package` writes `.protoss/packages/<hash>.package` plus `.protoss/package`, a deterministic content-addressed package descriptor over the lock hash, canonical format versions, program and graph hashes, public interface hash, declared `package_interfaces`, type aliases, DefIds, capability scopes, and source units. `project package --locked` requires the existing lockfile to match before writing; `project package --check` verifies the current package hash and descriptor against the lock, current canonical build, and any `package_interfaces = ["name=p1:..."]` constraints in `protoss.toml`.
