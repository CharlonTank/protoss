...
- Workspaces use `protoss.toml`; `project build` writes `.protoss/store` with canonical defs, `program.canon`, `program.graph.json`, content-addressed graph objects under `graphs/`, types, deps, normal forms, roots, build refs, and web markers. Content refs use the declared `sha256`/`p2:` hash contract, and stored graph objects can be listed with `store graphs` or read and validated with `store graph <graphHash>`.
