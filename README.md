...
- `dune runtest` runs a fast smoke suite for parser/checker/hash/normalization/cache/process/patch rollback. Longer suites are explicit: `dune build @coretest`, `dune build @integrationtest`, `dune build @stdlibtest`, `dune build @selftest`, or `dune build @fulltest` (aggregates the leaf section aliases — including the three workspace slices `@integrationtest-workspace-project`/`-consumer`/`-corruption` — and runs them as parallel processes).
